...
Single line comments start with a '#' or two slashes '//' and end with the end of the line.

#### Multi-Line Comment / Block Comments
Block comments are C-like and start with a slash and an asterisk '/*' and end with '*/'.

#### Attributes attached to comments
Comment lines starting with a **@** and an alphanumeric identifier that is not in the list of reserved attribute names (see [bridges to other analysis tools](bridges.md)).
Comment lines prefixed with such an attribute are embedded with the attribute name as key and the following lines until the next attribute declaration
into the Lua table generated by the mewa program option **--generate-language**. This mechanism allows mewa to interface with other tools that need
a language grammar enriched with new elements as source. The attributes allow to provide the information missing for the embedding.
The structure of the Lua table provided by the mewa program option **--generate-language** is described in detail in [bridges to other analysis tools](bridges.md).
