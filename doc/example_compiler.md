## Example Compiler Project "language1"

Here is a description of the example language also used in tests.

### Files
 - [Grammar Description](../examples/language1/grammar.g)
 - [First Class Citizen LLVM Templates](../examples/language1/fcc.lua)
 - [LLVM Templates](../examples/language1/llvmir.lua)
 - [Typesystem / Lua Callbacks Referenced in Grammar](../examples/language1/typesystem.lua)
 - [Example Source File](../examples/language1/source.prg)


 
