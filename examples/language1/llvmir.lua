...
local arrayTemplate = {
	...
	rewind_ctor = "%D1 = ptrtoint {element}* %ths to i64\n"
		...
		.. "%D4 = sub i64 %D3, %D2\n"
		.. "%D5 = sub i64 %D1, %D2\n"
		.. "%D6 = udiv exact i64 %D5, %D4\n"
		.. "%D7 = trunc i64 %D6 to i32\n"
		.. "call void @__dtor_{size}__{elementsymbol}( [{size} x {element}]* %ar, i32 %D7)",
...
llvmir.control = {
	...
	sretFunctionCallThrowing = "invoke void{signature} {func}( {rtllvmtype}* sret {reftyperef}{callargstr}) to label %{1} unwind label %{cleanup}\n{1}:\n",	
